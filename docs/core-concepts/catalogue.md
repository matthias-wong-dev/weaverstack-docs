...
Use Build, Load, Test, mirror and wipe as writers. Use Health, command output and read-only queries for inspection. Generated procedures and Weaver's internal write order are implementation details rather than additional catalogue contracts. The [Catalogue contract](../contracts/catalogue.md) defines publication, certification and compatibility boundaries; the [Load contract](../contracts/load.md) defines Load's execution and recording boundary.
