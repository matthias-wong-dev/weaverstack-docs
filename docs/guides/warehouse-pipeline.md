...
- Complete [First project](../get-started/first-project.md), or initialise a project with the logical item `Warehouse/Operations`:
...
Name selection runs exactly the named installed object. Use item-wide selection when Weaver should apply dependency ordering; the [Load contract](../contracts/load.md) defines both forms.
...
If a command rejects an option or object name, check the [CLI reference](../reference/cli.md). If Fabric access fails, use `weaver doctor` as described in [Installation](../get-started/installation.md). If the project lifecycle is unfamiliar, return to [First project](../get-started/first-project.md).
