...
This guide covers the routine inspect, plan, run and inspect loop after Build has installed a Weaver estate. Build, Load and Test retain the meanings defined by [Weaver operations](../core-concepts/build-load-and-test.md); this page concentrates on operator checkpoints.
...
Build has no dry-run option. It installs project source and can leave completed physical work in place if a later action fails. Keep its selection to the logical items that should be reconciled, then inspect Health again. See the [Development cycle](development-cycle.md) for the edit-and-Build loop.
...
Health's JSON includes a format version, overall status, sections and findings. Build, Load and Test also expose `--json`, but their fields are command-specific; do not infer one command's schema from another. Consult the [CLI reference](../reference/cli.md) and [Contracts](../reference/operation-behaviour/index.md) before binding automation to an output contract.
