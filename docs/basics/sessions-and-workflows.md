...
This is the complete checked fixture file. Each entry is one Weaver command line. A workflow is not a shell script: pipes, redirections, command substitution, environment expansion and command chaining are refused. A workflow cannot contain `session` or another `workflow`. Exact file grammar and command options belong in [CLI reference](../reference/cli.md) and [Contracts](../reference/operation-behaviour/index.md).

The file order is execution order. Here, Check validates source before Build installs it; Load runs the installed work; Test evaluates the resulting data; Health reads the recorded and physical state. [Weaver operations](../core-concepts/build-load-and-test.md) explains these boundaries.
...
Use the interactive Session first when diagnosis needs several attempts. Use [Run Weaver unattended](../advanced/automation-and-execution-contexts.md) for runner authentication, logs and machine-readable command output. Use [Promote a bundle](../advanced/build-bundles-and-controlled-installation.md) when Build planning and installation must happen in separate processes. The [Development cycle](development-cycle.md) describes how the same logical project moves through a development estate.
